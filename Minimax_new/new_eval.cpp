...
std::vector<std::pair<int, int>> attack_points() {
  //生成威胁或者进攻点位
  std::vector<std::pair<int, int>> res;
  ...
  for (int i = 0; i <= 14; i++) {
    for (int j = 0; j <= 14; j++) {
      ...
      for (int i = 0; i <= 3; i++) {
        ...
        while (is_valid_pair(r_end) && get(r_end) == 1 - ai_side) {
          length++;
          r_end = r_end + dir[i];
        }
        if (is_valid_pair(r_end) && get(r_end) == -1)
        ...
        while (is_valid_pair(l_end) && get(l_end) == 1 - ai_side) {
          length++;
          l_end = l_end - dir[i];
        }
        if (is_valid_pair(l_end) && get(l_end) == -1)
        ...
        if (length == 4 && !(flag_l == 1 && flag_r == 1) || length == 5) {
          res.push_back(p);
          is_in[i + 1000 * j] = 1;
        }
      }

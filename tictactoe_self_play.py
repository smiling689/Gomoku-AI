...
def self_play_draw_check(board_width=3, board_height=3, n_in_row=3, model_path=None, n_games=100):
    # 创建棋盘和游戏环境
    board = Board(width=board_width, height=board_height, n_in_row=n_in_row)
    game = Game(board)

    # 加载AI模型
    policy_value_net = PolicyValueNet(board_width, board_height, block=1, init_model=model_path, cuda=False)
    ...
    for i in range(n_games):
        winner = game.start_play(mcts_player, mcts_player, start_player=i%2, is_shown=0)
        if winner == -1:
            draws += 1
...
if __name__ == "__main__":
    ...
    self_play_draw_check(board_width=3, board_height=3, n_in_row=3, model_path=model_path, n_games=100)
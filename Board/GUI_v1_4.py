...
class GUI:
    ...
    def reset(self, bs):
        ...
        # Ensure screen width is enough for board and buttons
        board_width_px = self.BoardSize * self.UnitSize + 2 * self.UnitSize
        # Button widths: ResetScore (2.5), SwitchPlayer (3), RestartGame (3). Total 8.5. Add padding.
        buttons_width_px = (2.5 + 3 + 3 + 1) * self.UnitSize  # Using 1 UnitSize as padding between buttons
        screen_width = max(board_width_px, buttons_width_px)

        self.ScreenSize = (screen_width,
        ...
        # button areas
        buttons_y = self.ScreenSize[1] - self.UnitSize
        self.areas['ResetScore'] = Rect(0, buttons_y, self.UnitSize * 2.5, self.UnitSize)
        self.areas['SwitchPlayer'] = Rect(self.ScreenSize[0] / 2 - self.UnitSize * 1.5, buttons_y, self.UnitSize * 3, self.UnitSize)
        self.areas['RestartGame'] = Rect(self.ScreenSize[0] - self.UnitSize * 3, buttons_y, self.UnitSize * 3, self.UnitSize)

        # Calculate board position to center it horizontally
        board_length = self.UnitSize * self.BoardSize
        board_x = (self.ScreenSize[0] - board_length) / 2  # 计算使棋盘水平居中的x坐标
        self.areas['board'] = Rect(board_x, self.UnitSize, board_length, board_length)
                    ...
                    for name, rec in self.areas.items():
                        if self._in_area(mouse_pos, rec):
                            if name != 'board':
                                ...
                                # 获取棋盘区域的实际位置
                                board_rect = self.areas['board']
                                board_x = board_rect.left
                                board_y = board_rect.top

                                # 基于棋盘实际位置计算点击的格子坐标
                                relative_x = mouse_pos[0] - board_x
                                relative_y = mouse_pos[1] - board_y

                                x = int(relative_x // self.UnitSize)
                                y = self.BoardSize - int(relative_y // self.UnitSize) - 1
    ...
    def _draw_pieces(self, loc, player, last_step=False):
        ...
        pos = (int(board_x + self.UnitSize * (x + 0.5)), 
               int(board_y + self.UnitSize * (self.BoardSize - y - 0.5)))
    ...
    def _draw_static(self):
        ...
        board_y = board_rect.top
        board_length = self.UnitSize * self.BoardSize
        pygame.draw.rect(self.screen, self._board_color, board_rect)
        for i in range(self.BoardSize):
            # draw grid lines
            # 计算水平线和垂直线的起点和终点
            h_start_x = board_x + self.UnitSize * 0.5
            h_start_y = board_y + self.UnitSize * (i + 0.5)
            h_end_x = board_x + board_length - self.UnitSize * 0.5
            h_end_y = h_start_y
            
            v_start_x = board_x + self.UnitSize * (i + 0.5)
            v_start_y = board_y + self.UnitSize * 0.5
            v_end_x = v_start_x
            v_end_y = board_y + board_length - self.UnitSize * 0.5
            
            # 绘制水平和垂直的网格线
            pygame.draw.line(self.screen, (0, 0, 0), (v_start_x, v_start_y), (v_end_x, v_end_y))
            pygame.draw.line(self.screen, (0, 0, 0), (h_start_x, h_start_y), (h_end_x, h_end_y))
            
            # 绘制棋盘边框
            pygame.draw.rect(self.screen, (0, 0, 0), board_rect, 1)
            
            # 绘制坐标值
            self._draw_text(self.BoardSize - i - 1, (board_x - self.UnitSize * 0.5, board_y + self.UnitSize * (i + 0.5)), text_height=self.TestSize)  # 竖的
            self._draw_text(i, (board_x + self.UnitSize * (i + 0.5), board_y - self.UnitSize * 0.5), text_height=self.TestSize)  # 横的
